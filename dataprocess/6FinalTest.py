...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=r"C:\Users\DOCTOR\Desktop\daliy\DrZG\oiltank_ship_aug68",
    )

    parser.add_argument("--oil_class_id", type=int, default=1)
    parser.add_argument("--oil_class_name", type=str, default="油罐")

    parser.add_argument("--num_pairs", type=int, default=68)
    ...
    if len(oil_sources) < args.num_pairs:
        raise RuntimeError(f"Not enough oil tank samples. need={args.num_pairs}, got={len(oil_sources)}")
    ...
    for idx in range(args.num_pairs):
        oil_img_path, oil_txt_path = oil_sources[idx]
